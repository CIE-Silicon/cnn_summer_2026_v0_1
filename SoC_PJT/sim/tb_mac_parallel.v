...
// Updated 02.08.2026: each mac_unit inside mac_parallel is now pipelined,
// so y is valid 2 clock edges after a0..a8/w change, not 1 (mac_valid's
// own extra register stage already accounted for this - only this TB's
// manual @(posedge clk) sampling needed updating to match). Also
// corrected expected values: Mac1 (weight=-7) and Mac5 (weight=-3) always
// produce a negative raw sum for the inputs used here, so the ReLU stage
// clamps their y to 0; Mac10 (weight=2) and Mac16 (weight=8) stay
// positive and are unaffected, so their expected values are unchanged.
...
initial begin
	...
	@(posedge clk);
	#1;
	$display("TC1 : Mac1_y=%0d (Exp:0, raw=-315) | Mac5_y=%0d (Exp:0, raw=-135) | Mac10_y=%0d (Exp:90) | Mac16_y=%0d (Exp:360)", 
	...
	@(posedge clk);
	#1;
	$display("TC2 : Mac1_y=%0d (Exp:0, raw=-63)  | Mac5_y=%0d (Exp:0, raw=-27)  | Mac10_y=%0d (Exp:18) | Mac16_y=%0d (Exp:72)",  
	         $signed(y[15:0]), $signed(y[(4*16)+:16]), $signed(y[(9*16)+:16]), $signed(y[(15*16)+:16]));
	#50;
	...
	@(posedge clk);
	#1;
	$display("TC3 : Mac1_y=%0d (Exp:0, raw=-945) | Mac5_y=%0d (Exp:0, raw=-405) | Mac10_y=%0d (Exp:270)| Mac16_y=%0d (Exp:1080)", 
	         $signed(y[15:0]), $signed(y[(4*16)+:16]), $signed(y[(9*16)+:16]), $signed(y[(15*16)+:16]));
	#50;
	...
	@(posedge clk);
	#1;
	$display("TC4 : Mac1_y=%0d (Exp:0, raw=-16065)| Mac5_y=%0d (Exp:0, raw=-6885)| Mac10_y=%0d (Exp:4590)| Mac16_y=%0d (Exp:18360)", 
	         $signed(y[15:0]), $signed(y[(4*16)+:16]), $signed(y[(9*16)+:16]), $signed(y[(15*16)+:16]));
	#200;
